...
try:
    ...
    print('Module astral is not found. Use command pip3 install astral')

# Globals
###############################

LOCATION = 'Seattle'                                                                                                                    
BEGIN_BRIGHTNESS = dt.time(7,0)                                                                                                         
BEGIN_DIMMING = dt.time(18,0)                                                                                                           
BRIGHTNESS_LEVEL = 0.5 # Between 0-1                                                                                                    
SUNRISE_SUNSET = True
GUST_THRESHOLD = 10                                                                                                                     
FILE_LOCATION = './airports'                                                                                                            
...
def get_web_data(airports)->str:
    '''Pulls and returns XML web data from globally defined url'''
    url:str = URL_FOR_WEATHER + ','.join([item for item in airports if item != 'NULL'])
    print(f'Attempting to pull data from: {url}')
    req = urllib.request.Request(url, headers={'User-Agent': 'Mozilla/5.0 (Windows ' \
...
def get_weather_condition(head):
    '''From the data given by website, parses the XML and updates a dictionary of airport data

    An example of the XML format can be found in the globals.py file under URL_FOR_WEATHER.
    The conditions dictionary uses the station_id as the key and stores station weather.
    Stations used and the dictionary of weather are returned to calling function.
    '''
    conditions:dict = {}
    stations:list[str] = []
